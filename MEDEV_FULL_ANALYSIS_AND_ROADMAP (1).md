...
### 3.8 Groq-специфичные ограничения (важно знать)

Groq — не OpenAI. Ряд вещей работает иначе:

- **JSON mode:** поддерживается, но нестабильно на мелких моделях (llama-3.1-8b). Для production использовать `llama-3.3-70b` или `mixtral-8x7b`.
- **Tool calling:** поддерживается с `llama-3.1-70b` и выше. На 8b — экспериментально.
- **Rate limits на Free tier:** 30 req/min, 14400 req/day, 500k tokens/min — звучит много, но при streaming и long prompts лимит по токенам бьётся быстро.
- **Latency:** Groq быстрее OpenAI (inference на LPU), но cold start при первом запросе может быть 2–3s.
- **Нет встроенного token counting до запроса:** нужно считать самому (tiktoken / approximation `len(text)/4`).

[WARNING] Если Groq поднимет цены или изменит условия Free — всё упадёт. Проектируй `GroqClient` за интерфейсом `LlmProvider`, чтобы можно было подключить OpenRouter или Anthropic за 1 день.

```java
public interface LlmProvider {
    Mono<String> streamCompletion(LlmRequest request);
    Mono<JsonNode> structuredCompletion(LlmRequest request, JsonSchema schema);
}
...
Это не over-engineering — это страховка от vendor lock-in на уровне одного интерфейса.

### 3.9 Prompt Versioning — чего не хватает в большинстве MVP

Промпты — это код. Они должны быть версионированы и тестируемы.

Проблема: если промпт лежит прямо в Java-строке в сервисе, то:
- непонятно что изменилось между версиями
- нельзя откатиться
- нельзя A/B тестировать
- нельзя прогнать golden set на старой и новой версии

Минимальное решение для MeDev:

```
src/main/resources/prompts/
  summary_v1.txt
  project_description_v1.txt
  resume_analysis_v1.txt
```
...
public class PromptLoader {
    public String load(String name) {
        // читает из classpath:/prompts/{name}.txt
        // кешируется при старте
    }
}
```

При смене промпта — новый файл `summary_v2.txt`, старый не удаляется. Версия прописывается в конфиге (`ai.prompts.summary=summary_v2`). Golden set прогоняется на обеих версиях перед деплоем.
...
#### 4.1.6 Prompt Versioning

- [ ] Вынести все промпты в `src/main/resources/prompts/*.txt`
- [ ] `PromptLoader` — загрузка и кэш при старте
- [ ] Версия активного промпта конфигурируется через `application.yml`
- [ ] При изменении промпта — новый файл, старый не удалять

#### 4.1.7 Tools / Function Calling (следующий этап)
...
- [ ] Ввести интерфейс `LlmProvider` — изолировать Groq за абстракцией (vendor lock-in protection)
...
### 4.6 Security (явно не указано, но критично)

- [ ] Input sanitization на AI-эндпоинтах — защита от prompt injection через user-supplied данные
- [ ] Max prompt size limit (например 8000 токенов суммарно) — защита от DoS через огромный контекст
- [ ] Логировать промпты только в debug-режиме, не в production (могут содержать PII)
- [ ] GROQ_API_KEY rotation strategy — документировать как менять без downtime
- [ ] Audit log: кто когда делал AI-запрос (для billing disputes)
...
| Prompt injection через профиль    | Средняя     | Высокий| Input sanitization + size limits на user data  |
| Groq меняет условия / цены        | Средняя     | Высокий| Интерфейс LlmProvider, готовность к миграции   |
| Промпт деградирует без контроля   | Высокая     | Средний| Prompt versioning + golden set regression      |
...
- [ ] Ввести интерфейс `LlmProvider`, перенести Groq за него
- [ ] Timeout/retry/fallback в GroqClient
- [ ] Rate limiting на AI (Bucket4j)
- [ ] AiContextService + кэш GitHub (Redis, TTL 1h)
- [ ] Input sanitization + max size на user data в промпте

**Неделя 2**
- [ ] Вынести промпты в `resources/prompts/` + PromptLoader
- [ ] Structured output (JSON mode) + schema validation
- [ ] Token accounting (таблица `ai_usage`)
- [ ] Generate with AI кнопки в редакторе (summary + projects)
- [ ] Human feedback endpoint + UI-кнопки
...
## Приложение D. Ключевые архитектурные решения (ADR)

Зафиксированы здесь, чтобы не объяснять их заново в каждой сессии.

**ADR-001: SSE вместо WebSocket для AI-стриминга**
- Статус: принято
- Причина: full-duplex не нужен, SseEmitter в Spring Boot нативен, iOS Safari поддерживает без хаков
- Альтернатива: WebSocket — отклонена как излишняя

**ADR-002: Groq как primary LLM-провайдер**
- Статус: принято (временно)
- Причина: бесплатный tier, низкая latency за счёт LPU
- Риск: vendor lock-in, нестабильность free tier при росте
- Митигация: интерфейс `LlmProvider` — смена провайдера за 1 день

**ADR-003: JSON mode вместо tool calling на первой итерации**
- Статус: принято
- Причина: tool calling нестабилен на llama-3.1-8b (Groq free tier), JSON mode проще в реализации и отладке
- Переход: на tool calling после стабилизации и при наличии тестов

**ADR-004: Промпты в `resources/prompts/`, не в Java-строках**
- Статус: принято (к реализации в Фазе 1)
- Причина: промпты — это код, нужны versioning, тестирование и возможность отката
