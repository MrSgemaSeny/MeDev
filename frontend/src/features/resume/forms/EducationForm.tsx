...
export function EducationForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId === 'new') {
      ...
      updateEducation({ id: editingId, payload: formData }, {
        onSuccess: () => {
      ...
      {!editingId ? (
        <div className="flex-1 overflow-y-auto pr-2 space-y-3 pb-20">
          {profile?.education?.map((edu: any) => (
            <div key={edu.id} className="p-3 rounded-md border flex items-start justify-between" style={{ borderColor: 'var(--color-border-default)', backgroundColor: 'var(--color-bg-secondary)' }}>
              <div>
                <div className="font-medium text-sm" style={{ color: 'var(--color-text-primary)' }}>{edu.institution}</div>
                <div className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{edu.degree} {edu.field && `in ${edu.field}`}</div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => handleEdit(edu)} className="p-1 hover:opacity-70 transition-opacity" style={{ color: 'var(--color-text-secondary)' }}>
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(edu.id)} className="p-1 hover:text-red-500 transition-colors" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto pr-2 space-y-4 pb-20">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'var(--color-text-secondary)' }}>Institution</label>
            <input name="institution" value={formData.institution} onChange={handleChange} className={inputClass} style={inputStyle} required />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'var(--color-text-secondary)' }}>Degree</label>
            <input name="degree" value={formData.degree} onChange={handleChange} className={inputClass} style={inputStyle} required placeholder="Bachelor of Science" />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'var(--color-text-secondary)' }}>Field of Study</label>
            <input name="field" value={formData.field} onChange={handleChange} className={inputClass} style={inputStyle} placeholder="Computer Science" />
          </div>
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" name="isCurrent" id="isCurrentEdu" checked={formData.isCurrent} onChange={handleChange} className="rounded" />
            <label htmlFor="isCurrentEdu" className="text-xs font-medium" style={{ color: 'var(--color-text-secondary)' }}>I currently study here</label>
          </div>
          <div>
