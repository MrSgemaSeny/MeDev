    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-16 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-10 sm:p-14 shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-200/60 dark:border-slate-800/60 text-center relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-br from-indigo-500/20 via-purple-500/20 to-pink-500/20 dark:from-indigo-500/10 dark:via-purple-500/10 dark:to-pink-500/10 blur-3xl rounded-full translate-y-[-50%] group-hover:scale-110 transition-transform duration-700"></div>
          ...
          <div className="relative z-10">
            {profile.avatarUrl ? (
              ...
              <div className="w-32 h-32 rounded-3xl bg-gradient-to-tr from-indigo-100 to-purple-100 dark:from-slate-800 dark:to-slate-700 mx-auto mb-6 flex items-center justify-center shadow-xl shadow-indigo-500/10 dark:shadow-black/50 border-4 border-white dark:border-slate-800">
                <span className="text-4xl font-extrabold text-indigo-500 dark:text-indigo-400">{profile.fullName?.charAt(0) || username?.charAt(0)}</span>
              </div>
            )}
            ...
            <p className="text-indigo-600 dark:text-indigo-400 font-bold text-lg">{profile.headline}</p>
        ...
        {profile.experience && profile.experience.length > 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 sm:p-12 shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-200/60 dark:border-slate-800/60 relative">
            <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-12 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center text-blue-600 dark:text-blue-400">💼</div>
              Experience
            </h2>
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-700 before:to-transparent">
              {profile.experience.map((exp: any) => (
                <div key={exp.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-slate-900 bg-slate-200 dark:bg-slate-700 group-hover:bg-indigo-500 group-hover:border-indigo-100 dark:group-hover:border-indigo-900/50 text-slate-500 group-hover:text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 transition-colors duration-300 absolute md:relative left-0 md:left-auto"></div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/50 dark:border-slate-700/50 group-hover:shadow-lg group-hover:border-indigo-500/30 dark:group-hover:border-indigo-500/30 transition-all duration-300 ml-16 md:ml-0">
                    <h3 className="font-bold text-lg text-slate-900 dark:text-white">{exp.position}</h3>
                    <div className="text-indigo-600 dark:text-indigo-400 font-semibold text-sm mb-3">{exp.company}</div>
        ...
        {profile.projects && profile.projects.length > 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 sm:p-12 shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-200/60 dark:border-slate-800/60">
            <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-8 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-500/20 flex items-center justify-center text-purple-600 dark:text-purple-400">🚀</div>
              Featured Projects
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {profile.projects.filter((p: any) => p.isFeatured).map((proj: any) => (
                <div key={proj.id} className="group border border-slate-200/60 dark:border-slate-700/60 rounded-2xl p-6 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-white dark:hover:bg-slate-800 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-none hover:border-indigo-500/30 dark:hover:border-indigo-500/30 transition-all duration-300 flex flex-col h-full">
                  <h3 className="font-extrabold text-lg text-slate-900 dark:text-white mb-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{proj.name}</h3>
                  ...
                  <div className="flex gap-4 text-sm font-semibold mt-auto pt-4 border-t border-slate-200/60 dark:border-slate-700/60">
                    {proj.githubUrl && (
                      <a href={proj.githubUrl} target="_blank" rel="noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center gap-1">
                    ...
                    {proj.liveUrl && (
                      <a href={proj.liveUrl} target="_blank" rel="noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center gap-1">
