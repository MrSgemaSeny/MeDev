...
export const Pricing = () => {
  return (
    <section id="pricing" className="border-b border-[#30363d] bg-[#0d1117] py-10 sm:py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-3.5 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-6 sm:mt-12 grid max-w-lg grid-cols-1 gap-4 sm:gap-6 lg:max-w-5xl lg:grid-cols-2">
          {tiers.map((tier) => (
            <div
              key={tier.id}
              className={`flex flex-col justify-between rounded-xl sm:rounded-2xl p-4 sm:p-7 lg:p-9 transition-all ${
                tier.featured
            ...
            >
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-[#f0f6fc]">{tier.name}</h3>

                <div className="mt-3 sm:mt-5 flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
                  <span className="text-3xl sm:text-4xl font-extrabold tracking-tight text-[#f0f6fc]">
                    {tier.price}
                  </span>
                  <span className="text-xs sm:text-sm font-semibold text-[#8b949e]">{tier.period}</span>
                  {tier.localPrice && (
                    <span className="text-[11px] sm:text-xs text-[#8b949e]">({tier.localPrice})</span>
                  )}
                </div>

                <p className="mt-2 sm:mt-3 text-xs sm:text-sm leading-relaxed text-[#c9d1d9]">{tier.description}</p>

                <div className="mt-4 sm:mt-6 border-t border-[#30363d] pt-4 sm:pt-6">
                  <ul className="space-y-2 sm:space-y-3">
                    {tier.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-2.5 text-xs sm:text-sm text-[#c9d1d9]">
                        <Check className="h-4 w-4 shrink-0 text-[#2ea043] mt-0.5" aria-hidden="true" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="mt-5 sm:mt-8 pt-4 sm:pt-6 border-t border-[#30363d]">
                <a
                  href={tier.href}
                  className={`flex w-full min-h-[40px] sm:min-h-[44px] items-center justify-center rounded-lg sm:rounded-xl px-4 py-2.5 sm:py-3.5 text-xs sm:text-base font-bold transition-all focus-visible:ring-2 focus-visible:ring-[#2ea043] focus-visible:outline-none ${
                    tier.featured
